 DROP MATERIALIZED VIEW pension_data_all; -- temp;

CREATE MATERIALIZED VIEW pension_data_all AS -- temp AS 
...
-- 	end as asset_location,
    ...
    -- --------------------------------------------------		case
        CASE
            WHEN p.instrument_sub_type::text = ANY (ARRAY['חוזים עתידיים'::character varying, 'אופציות'::character varying]::text[]) THEN p.instrument_sub_type
--            ELSE COALESCE(ext_data.instrument_name, p.instrument_symbol, p.instrument_id)
            ELSE COALESCE(ins_data.instrument_name, ext_data.instrument_name, p.instrument_symbol, p.instrument_id)
