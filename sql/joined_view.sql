CREATE OR REPLACE VIEW pension_data_all AS 
 SELECT debug.managing_body, 
    ...
    debug.currency, 
    COALESCE(debug.fair_value, 0::numeric) * 1000::numeric AS fair_value, 
    COALESCE(debug.market_cap, 0::numeric) * 1000::numeric AS market_cap, 
    debug.rate_of_fund, 
    debug.rating_agency, 
    ext.reference_index, 
    debug.intrest_rate, 
  ...
  WHERE debug.instrument_id::text <> ''::text AND debug.instrument_id IS NOT NULL AND debug.instrument_symbol::text <> ''::text AND debug.instrument_symbol IS NOT NULL;
